...
print("\nCase: Laplacian using Discordance as adjacency\n")
...
def Binline_L_with_Discordance(x):
	"""
	...
	c1 = np.dot(Xl.T, d_inv_x)
	c1 = Xu.dot(c1) / (T-1.0)
	c2 = np.dot(Xu.T, d_inv_x)
	c2 = Xl.dot(c2) / (T-1.0)
	C_d_x = c1 + c2

	# build the "d^(-0.5) A d^(-0.5) x" part
	D_inv_C_D_inv_x = np.multiply(d_inv.ravel(), -C_d_x) # correct sign of adjacency (-Disc)
...
Lin_oper = LinearOperator(shape=(Xu.shape[0], Xu.shape[0]) , matvec =  Binline_L_with_Discordance)

#* Get a subset of the spectrum of matrix L = I - D^(-0.5) A D^(-0.5)
print(' '*3+"Computing the first {} largest eigvals and eigenvecs of L\n".format(k))
...
idx = vals.argsort()[::-1] # order from largest to smallest
...
# np.save(save_to_ + "GROUP_vecs_{}_laplace_disc".format(k),vecs)
# np.save(save_to_ + "GROUP_vals_{}_laplace_disc".format(k),vals)
...
#* The leading laplacian eigenvector
...
mynii_name = "leading_eigenv_laplace_disc.nii"                      # do not forget the file extension
